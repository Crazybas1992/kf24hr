...
class Vip(commands.Cog):
    ...
    async def vip_check(
        ...
        # ทำให้ vip_id เป็นพิมพ์ใหญ่ทั้งหมดและเอาเครื่องหมาย '-' ออก
        normalized_vip_id = vip_id.replace('-', '').upper()
        ...
        # ค้นหาข้อมูลตามรหัสที่ระบุในคำสั่ง โดยแปลงเป็นตัวพิมพ์เล็กทั้งหมด
        vip_id = vip_id.lower().replace("-", "")  # แปลงเป็นตัวพิมพ์เล็กและลบขีด

        found = False
        for row in data[1:]:  # เริ่มจากแถวที่ 1 เพราะแถวที่ 0 คือหัวข้อ
            sheet_vip_id = row[0].lower().replace("-", "")  # แปลงเป็นตัวพิมพ์เล็กและลบขีด
            if sheet_vip_id == vip_id:  # เปรียบเทียบโดยไม่สนใจช่องว่าง
                # ส่วนของโค้ดสร้าง Embed เพื่อแสดงผลข้อมูล
                embed = discord.Embed(
                    title="VIP - Credit : 💳",
        ...
        if not found:
            await ctx.respond(f"ไม่พบข้อมูลสำหรับรหัส VIP หรือตรวจสอบว่าคุณได้พิมพ์ถูกต้องแล้ว: {vip_id}", ephemeral=True)
